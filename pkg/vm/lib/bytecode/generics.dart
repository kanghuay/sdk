...
List<DartType> getTypeParameterTypes(List<TypeParameter> typeParameters) {
  ...
  List<DartType> types = List.generate(typeParameters.length, (i) => new TypeParameterType(
        typeParameters[i], TypeParameterType.computeNullabilityFromBound(typeParameters[i])));
  return types;
}
...
bool isSealedType(DartType type, CoreTypes coreTypes) {
  if (type is InterfaceType) {
    ...
    return cls == coreTypes.intClass ||
        ...
        cls == coreTypes.deprecatedNullType;
  }
  return false;
}
...
bool isUncheckedClosureCall(InstanceInvocationExpression node,

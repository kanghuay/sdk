...
class BufferedWriter {
  ...
  void writeLinkOffset(Object? target) {
    final offset = linkWriter.getOffset(target as BytecodeDeclaration);
...
class BufferedReader {
  ...
  List<T> readPackedList<T extends BytecodeObject>() {
    ...
    final list = List.generate(len, (index) => readPackedObject() as T);
    return list;
  }
...
class StringTable implements StringWriter, StringReader {
  ...
  StringTable.read(BufferedReader reader) {
    ...
    final List<int> oneByteEndOffsets = new List<int>.filled(numOneByteStrings, 0);
    for (int i = 0; i < oneByteEndOffsets.length; ++i) {
      oneByteEndOffsets[i] = reader.readUInt32();
    }
    List<int> twoByteEndOffsets = new List<int>.filled(numTwoByteStrings, 0);
    for (int i = 0; i < twoByteEndOffsets.length; ++i) {
      twoByteEndOffsets[i] = reader.readUInt32();
    }
    int start = 0;
    if (numOneByteStrings > 0) {
      _oneByteStrings = new List<String>.filled(numOneByteStrings, "");
    ...
    if (numTwoByteStrings > 0) {
      int start = 0;
      _twoByteStrings = new List<String>.filled(numTwoByteStrings, "");

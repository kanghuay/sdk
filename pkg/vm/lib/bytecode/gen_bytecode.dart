...
import 'package:front_end/src/fasta/fasta_codes.dart' as fc show Code, MessageCode, Template, Message;
import 'package:front_end/src/fasta/kernel/type_labeler.dart' show TypeLabeler;
...
import 'object_table.dart'
    show NameAndType, NameHandle, ObjectHandle, ObjectTable, TypeHandle, topLevelClassName;
...
const fc.MessageCode messageBytecodeLimitExceededTooManyArguments =
const fc.MessageCode("BytecodeLimitExceededTooManyArguments",
    problemMessage: r"""Dart bytecode limit exceeded: too many arguments.""");

const fc.Code<fc.Message Function(DartType _type, bool isNonNullableByDefault)>
codeIllegalRecursiveType =
const fc.Code<fc.Message Function(DartType _type, bool isNonNullableByDefault)>(
  "IllegalRecursiveType",
);

fc.Message _withArgumentsIllegalRecursiveType(
    DartType _type, bool isNonNullableByDefault) {
  TypeLabeler labeler = new TypeLabeler(isNonNullableByDefault);
  List<Object> typeParts = labeler.labelType(_type);
  String type = typeParts.join();
  return new fc.Message(codeIllegalRecursiveType,
      problemMessage: """Illegal recursive type '${type}'.""" + labeler.originMessages,
      arguments: {'type': _type});
}
const fc.Template<fc.Message Function(DartType _type, bool isNonNullableByDefault)>
templateIllegalRecursiveType = const fc.Template<
    fc.Message Function(DartType _type, bool isNonNullableByDefault)>(
    problemMessageTemplate: r"""Illegal recursive type '#type'.""",
...
class BytecodeGenerator extends RecursiveVisitor<Null> {
  ...
  late NullabilityDetector nullabilityDetector;
  late Map<Node, DirectCallMetadata> directCallMetadata;
  ...
  Map<Node, InferredType>? inferredTypeMetadata;
  ...
  late List<ObjectHandle> nullableFields;
  late ConstantPool cp;
  late BytecodeAssembler asm;
  List<BytecodeAssembler>? savedAssemblers;
  late bool hasErrors;
  late int currentLoopDepth;
  late List<int> savedMaxSourcePositions;
  late int maxSourcePosition;
  ...
  BytecodeGenerator._internal(
    ...
    directCallMetadata =
        component.metadata[DirectCallMetadataRepository.repositoryTag]?.mapping as Map<Node, DirectCallMetadata>;

    procedureAttributesMetadataRepository =
        component.metadata[ProcedureAttributesMetadataRepository.repositoryTag] as ProcedureAttributesMetadataRepository?;

    inferredTypeMetadata = component
        .metadata[InferredTypeMetadataRepository.repositoryTag]?.mapping as Map<Node, InferredType>?;
  ...
  LibraryDeclaration getLibraryDeclaration(
    ...
    for (var dependency in library.dependencies) {
      ...
      if (targetLibrary == coreTypes.mirrorsLibrary) {
       ...
       } else if (targetLibrary.name == 'dart.ffi') {
  ...
  Annotations getFunctionAnnotations(Member member) {
    ...
    for (final parameterNodes in parameterNodeLists) {
      List<Constant> parameterConstants =
          parameterNodes.map<Constant>(_getConstant).toList();
  ...
  defaultMember(Member node) {
    ...
    try {
      ...
      } else if ((node is Procedure && !node.isRedirectingFactory) ||
          (node is Constructor)) {
        if (hasCode) {
          ...
          if (node.isExternal) {
            final String? nativeName = getExternalName(coreTypes, node);
            if (nativeName != null) {
  ...
  bool _hasCode(Member member) {
    if (member is Procedure && member.isRedirectingFactory) {
      return false;
    }
  ...
  Procedure get listFromLiteral => _listFromLiteral ??=
      libraryIndex.getMember('dart:core', '_GrowableList', '_literal') as Procedure;

  Procedure? _mapFromLiteral;
  Procedure get mapFromLiteral => _mapFromLiteral ??=
      libraryIndex.getMember('dart:core', 'Map', '_fromLiteral') as Procedure;

  Procedure? _interpolateSingle;
  Procedure get interpolateSingle => _interpolateSingle ??=
      libraryIndex.getMember('dart:core', '_StringBase', '_interpolateSingle') as Procedure;

  Procedure? _interpolate;
  Procedure get interpolate => _interpolate ??=
      libraryIndex.getMember('dart:core', '_StringBase', '_interpolate') as Procedure;
  ...
  Procedure get objectInstanceOf => _objectInstanceOf ??=
      libraryIndex.getMember('dart:core', 'Object', '_instanceOf') as Procedure;

  Procedure? _objectSimpleInstanceOf;
  Procedure get objectSimpleInstanceOf => _objectSimpleInstanceOf ??=
      libraryIndex.getMember('dart:core', 'Object', '_simpleInstanceOf') as Procedure;
  ...
  Field get closureInstantiatorTypeArguments =>
      _closureInstantiatorTypeArguments ??= libraryIndex.getMember(
          'dart:core', '_Closure', '_instantiator_type_arguments') as Field;
  ...
  Field get closureFunctionTypeArguments =>
      _closureFunctionTypeArguments ??= libraryIndex.getMember(
          'dart:core', '_Closure', '_function_type_arguments') as Field;
  ...
  Field get closureDelayedTypeArguments =>
      _closureDelayedTypeArguments ??= libraryIndex.getMember(
          'dart:core', '_Closure', '_delayed_type_arguments') as Field;

  Field? _closureFunction;
  Field get closureFunction => _closureFunction ??=
      libraryIndex.getMember('dart:core', '_Closure', '_function') as Field;

  Field? _closureContext;
  Field get closureContext => _closureContext ??=
      libraryIndex.getMember('dart:core', '_Closure', '_context') as Field;

  Procedure? _prependTypeArguments;
  Procedure get prependTypeArguments => _prependTypeArguments ??=
      libraryIndex.getTopLevelMember('dart:_internal', '_prependTypeArguments') as Procedure;
  ...
  Procedure get boundsCheckForPartialInstantiation =>
      _boundsCheckForPartialInstantiation ??= libraryIndex.getTopLevelMember(
          'dart:_internal', '_boundsCheckForPartialInstantiation') as Procedure;

  Procedure? _futureValue;
  Procedure get futureValue =>
      _futureValue ??= libraryIndex.getMember('dart:async', 'Future', 'value') as Procedure;
  ...
  Procedure get throwNewLateInitializationError =>
      _throwNewLateInitializationError ??= libraryIndex.getMember(
          'dart:_internal', 'LateError', '_throwLocalNotInitialized') as Procedure;

  Procedure? _throwNewAssertionError;
  Procedure get throwNewAssertionError => _throwNewAssertionError ??=
      libraryIndex.getMember('dart:core', '_AssertionError', '_throwNew') as Procedure;
  ...
  Procedure get allocateInvocationMirror =>
      _allocateInvocationMirror ??= libraryIndex.getMember(
          'dart:core', '_InvocationMirror', '_allocateInvocationMirror') as Procedure;

  Procedure? _unsafeCast;
  Procedure get unsafeCast => _unsafeCast ??=
      libraryIndex.getTopLevelMember('dart:_internal', 'unsafeCast') as Procedure;

  Procedure? _iterableIterator;
  Procedure get iterableIterator => _iterableIterator ??=
      libraryIndex.getMember('dart:core', 'Iterable', 'get:iterator') as Procedure;

  Procedure? _iteratorMoveNext;
  Procedure get iteratorMoveNext => _iteratorMoveNext ??=
      libraryIndex.getMember('dart:core', 'Iterator', 'moveNext') as Procedure;

  Procedure? _iteratorCurrent;
  Procedure get iteratorCurrent => _iteratorCurrent ??=
      libraryIndex.getMember('dart:core', 'Iterator', 'get:current') as Procedure;
  ...
  Procedure get asyncAwaitCompleterGetFuture =>
      _asyncAwaitCompleterGetFuture ??= libraryIndex.tryGetMember(
          'dart:async', '_AsyncAwaitCompleter', 'get:future') as Procedure;

  Procedure? _setAsyncThreadStackTrace;
  Procedure get setAsyncThreadStackTrace => _setAsyncThreadStackTrace ??=
      libraryIndex.getTopLevelMember('dart:async', '_setAsyncThreadStackTrace') as Procedure;
  ...
  Procedure get clearAsyncThreadStackTrace =>
      _clearAsyncThreadStackTrace ??= libraryIndex.getTopLevelMember(
          'dart:async', '_clearAsyncThreadStackTrace') as Procedure;

  Library? _dartFfiLibrary;
  Library get dartFfiLibrary =>
      _dartFfiLibrary ??= libraryIndex.tryGetLibrary('dart:ffi') as Library;
  ...
  void _genArguments(TreeNode? receiver, Arguments arguments,
      {int? storeReceiverToLocal}) {
    if (arguments.types.isNotEmpty) {
      _genTypeArguments(arguments.types);
    }
    if (receiver != null && receiver is VariableDeclaration) {
      _genLoadVar(receiver);
    } else {
  ...
  void _appendInferredType(TreeNode node, int pc) {
    ...
    if (concreteClass != null && concreteClass != closureClass) {
      inferredTypesAttribute!.add(TypeLiteralConstant(coreTypes.rawType(
          concreteClass,
          (concreteClass == coreTypes.deprecatedNullType)
        ...
        if (node is Constructor) {
          ...
          if (node.isForwardingStub) {
            forwardingStubTargetCpIndex =
                cp.addObjectRef(node.concreteForwardingStubTarget);
          }
  ...
  Member? _getForwardingStubSuperTarget() {
    if (!isClosure) {
      ...
      if (member!.isInstanceMember &&
        ...
        return member.concreteForwardingStubTarget;
      }
    }
    return null;
  }
  ...
  ClosureDeclaration getClosureDeclaration(LocalFunction node,
    ...
    for (var v in function.positionalParameters) {
      parameters.add(new NameAndType(objectTable.getPublicNameHandle(v.name) as NameHandle,
          objectTable.getHandle(v.type) as TypeHandle));
    }
  ...
  bool _isUncheckedCall(
      ...
      (inferredTypeMetadata != null && node != null &&
  ...
  void _genInstanceCall(
    ...
    if (inferredTypeMetadata != null && node != null) {
      _appendInferredType(node as TreeNode, asm.offset);
    }
    ...
    if (inferredTypeMetadata != null && node != null) {
      _replaceWithConstantValue(node as TreeNode);
    }
  }
  ...
  visitInstanceInvocation(InstanceInvocation node) {
    //print(node);
    final directCall = directCallMetadata[node];
    ...
    if (!(interfaceTarget is Procedure && !interfaceTarget.isGetter)) {
      throw new UnsupportedOperationError(
          'Unsupported InstanceInvocation with interface target ${interfaceTarget.runtimeType} $interfaceTarget');
    }
    ...
    if (directCall != null) {
      final isDynamicForwarder = false;
      final isUnchecked =
  ...
  visitLocalFunctionInvocation(LocalFunctionInvocation node) {
    ...
    final int receiverTemp = locals.tempIndexInFrame(node);
    _genArguments(node.variable, args, storeReceiverToLocal: receiverTemp);
    // Duplicate receiver (closure) for UncheckedClosureCall.
    asm.emitPush(receiverTemp);
    asm.emitUncheckedClosureCall(argDescCpIndex, totalArgCount);
  ...
  visitDynamicGet(DynamicGet node) {
    ...
    if (directCall != null) {
      ...
      _genInstanceCall(node, InvocationKind.getter, null,
          node.name, node.receiver, 1, argDesc);
    }
  }
  ...
  visitDynamicSet(DynamicSet node) {
    ...
    _genInstanceCall(node, InvocationKind.setter, null,
  ...
  visitDynamicInvocation(DynamicInvocation node) {
    ...
    if (directCall != null) {
      final isDynamicForwarder = true;
      final isUnchecked =
          _isUncheckedCall(node, null, node.receiver);
      _genDirectCall(directCall.target, argDesc, totalArgCount,
          isDynamicForwarder: isDynamicForwarder,
          isUnchecked: isUnchecked,
          node: node);
    } else {
      _genInstanceCall(node, InvocationKind.method, null, node.name,
          node.receiver, totalArgCount, argDesc);
    }
  }
  ...
  visitEqualsCall(EqualsCall node) {
    ...
    final argDesc = objectTable.getArgDescHandle(2);
    final Member interfaceTarget = node.interfaceTarget;
    if (!(interfaceTarget is Procedure && interfaceTarget.kind == ProcedureKind.Operator)) {
      throw new UnsupportedOperationError(
          'Unsupported visitEqualsCall with interface target ${interfaceTarget.runtimeType} $interfaceTarget');
    }
    final directCall = directCallMetadata[node];
    if (directCall != null) {
      print(node);
    } else {
      _genInstanceCall(node, InvocationKind.method, node.interfaceTarget,
           new Name("=="), node.left, 2, argDesc);
    }
  }
  ...
  visitStaticGet(StaticGet node) {
    ...
    if (target is Field) {
      ...
      if (target.isGetter) {
        _genDirectCall(target, objectTable.getArgDescHandle(0), 0,
            isGet: true, node: node);
      } else if (target.isFactory || target.isRedirectingFactory) {
        throw 'Unexpected target for StaticGet: factory $target';
      } else {
        asm.emitPushConstant(cp.addObjectRef(new StaticTearOffConstant(target)));
      }
    } else {
  ...
  visitVariableGet(VariableGet node) {
    ...
    if (v.isConst) {
      ...
      if (v.initializer != null) {
        ...
        assert(init is InstanceInvocation &&
            init.name.text == "call" &&

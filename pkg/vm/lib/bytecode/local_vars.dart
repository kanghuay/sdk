...
const int asyncFutureContextIndex = 1;
...
class LocalVariables {
  ...
  final Map<Node, DirectCallMetadata> directCallMetadata;
...
class _ScopeBuilder extends RecursiveVisitor<Null> {
  ...
  static final registeredSyntheticVars = RegExp(r'^(' +
      ...
      '${ContinuationVariables.asyncFuture}|' +
    ...
    if (node is Field) {
      ...
      if (_currentFrame!.dartAsyncMarker == AsyncMarker.Async ||
        ...
        if (_currentFrame!.dartAsyncMarker == AsyncMarker.Async) {
          locals
              ._getVarDesc(_currentFrame!
                  .getSyntheticVar(ContinuationVariables.asyncFuture))
              .moveToScope(_currentScope!);
  ...
  void _captureAllVisibleVariablesInCurrentFrame() {
    ...
    transient
      ..addAll([
        if (_currentFrame!.functionTypeArgsVar != null) _currentFrame!.functionTypeArgsVar!,
        if (_currentFrame!.closureVar != null) _currentFrame!.closureVar!,
        if (_currentFrame!.contextVar!= null) _currentFrame!.contextVar!,
        if (_currentFrame!.scratchVar != null) _currentFrame!.scratchVar!,
        if (_currentFrame!.returnVar != null) _currentFrame!.returnVar!,
      ]);
...
class _Allocator extends RecursiveVisitor<Null> {
    ...
    if (node is Field) {
      ...
      if (_currentFrame!.dartAsyncMarker == AsyncMarker.Async) {
        final asyncCompleter =
            _currentFrame!.getSyntheticVar(ContinuationVariables.asyncFuture);
  ...
  visitVariableDeclaration(VariableDeclaration node) {
    ...
    } else if (node.name == ContinuationVariables.asyncFuture) {

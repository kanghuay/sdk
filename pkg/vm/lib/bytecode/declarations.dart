...
import 'object_table.dart' show NameAndType, NameHandle, ObjectHandle, ObjectTable, TypeHandle;
...
class LibraryDeclaration extends BytecodeDeclaration {
  ...
  factory LibraryDeclaration.read(BufferedReader reader) {
    ...
    final classes =
      ...
      return reader.readLinkOffset<ClassDeclaration>()..name = className as ObjectHandle;
    });
    final extensionUris = ((flags & hasExtensionsFlag) != 0)
        ? reader.readPackedList<ObjectHandle>()
        : const <ObjectHandle>[];
    return new LibraryDeclaration(
        null, flags, name as ObjectHandle, script as ObjectHandle, extensionUris, classes);
  }
...
class ClassDeclaration extends BytecodeDeclaration {
  ...
  factory ClassDeclaration.read(BufferedReader reader) {
    ...
    return new ClassDeclaration(
        null,
        flags,
        script as ObjectHandle,
        position,
        endPosition,
        typeParameters,
        numTypeArguments,
        superType as ObjectHandle?,
        interfaces,
        members,
        annotations);
  }
...
class SourceFile extends BytecodeDeclaration {
  ...
  factory SourceFile.read(BufferedReader reader) {
    ...
    final source = ((flags & hasSourceFlag) != 0)
        ...
        : null;
    return new SourceFile(importUri as ObjectHandle, lineStarts, source);
  }
...
class FieldDeclaration {
  ...
  factory FieldDeclaration.read(BufferedReader reader) {
    ...
    return new FieldDeclaration(
        flags,
        name as ObjectHandle,
        type as ObjectHandle?,
        value as ObjectHandle?,
        script as ObjectHandle?,
        position,
        endPosition,
        getterName as ObjectHandle?,
        setterName as ObjectHandle?,
        initializerCode,
        annotations,
        attributes as ObjectHandle?);
  }
...
class FunctionDeclaration {
  ...
  factory FunctionDeclaration.read(BufferedReader reader) {
    ...
    return new FunctionDeclaration(
        ...
        parameters,
        parameterFlags,
        returnType as ObjectHandle?,
        nativeName as ObjectHandle?,
        code,
        annotations,
        attributes as ObjectHandle?);
  }
...
class TypeParametersDeclaration {
  ...
  factory TypeParametersDeclaration.read(BufferedReader reader) {
    ...
    List<ObjectHandle> names = new List<ObjectHandle>.generate(
        numTypeParams, (_) => reader.readPackedObject() as ObjectHandle);
    ...
    return new TypeParametersDeclaration(new List<NameAndType>.generate(
        numTypeParams, (int i) => new NameAndType(names[i] as NameHandle, bounds[i] as TypeHandle)));
  }
...
class ParameterDeclaration {
  ...
  factory ParameterDeclaration.read(BufferedReader reader) {
    final name = reader.readPackedObject();
    final type = reader.readPackedObject();
    return new ParameterDeclaration(name as ObjectHandle, type as ObjectHandle);
  }
...
class AnnotationsDeclaration extends BytecodeDeclaration {
  ...
  factory AnnotationsDeclaration.read(BufferedReader reader) {
    return new AnnotationsDeclaration(reader.readPackedObject() as ObjectHandle?);
  }
...
class Component {
  ...
  Component.read(BufferedReader reader) {
    ...
    for (int i = 0; i < librariesNum; ++i) {
      ...
      library.importUri = importUri as ObjectHandle?;
    }

    if (mainOffset != 0) {
      reader.offset = start + mainOffset;
      mainLibrary = reader.readPackedObject() as ObjectHandle?;
    }
  }

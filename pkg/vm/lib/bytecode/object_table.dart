  ...
  factory ObjectHandle._empty(ObjectKind kind, int flags) {
    switch (kind) {
      ...
      case ObjectKind.kName:
        return ((flags & NameHandle.flagIsPublic) != 0)
      ...
      case ObjectKind.kType:
        Nullability nullability = Nullability
            .values[(flags & TypeHandle.nullabilityMask) ~/ flagBit4];
        switch (TypeTag.values[(flags & TypeHandle.tagMask) ~/ flagBit0]) {
...
class _LibraryHandle extends ObjectHandle {
  ...
  @override
  void readContents(BufferedReader reader) {
    uri = reader.readPackedObject() as _ConstObjectHandle;
  }
...
class _ClassHandle extends ObjectHandle {
  ...
  @override
  void readContents(BufferedReader reader) {
    library = reader.readPackedObject() as _LibraryHandle;
    name = reader.readPackedObject() as NameHandle;
  }
...
class _MemberHandle extends ObjectHandle {
  ...
  @override
  void readContents(BufferedReader reader) {
    parent = reader.readPackedObject() as _ClassHandle;
    name = reader.readPackedObject() as NameHandle;
  }
...
class _ClosureHandle extends ObjectHandle {
  ...
  @override
  void readContents(BufferedReader reader) {
    enclosingMember = reader.readPackedObject() as _MemberHandle;
...
abstract class TypeHandle extends ObjectHandle {
  ...
  TypeHandle(this.tag, this.nullability);
  ...
  set flags(int value) {
    if (value != flags) {
      throw 'Unable to set flags for TypeHandle (they are occupied by type tag and nnbd)';
    }
  }
}

class _DynamicTypeHandle extends TypeHandle {
...
class _VoidTypeHandle extends TypeHandle {
...
class _NeverTypeHandle extends TypeHandle {
...
class _NullTypeHandle extends TypeHandle {
  _NullTypeHandle()
      : super(TypeTag.kNull, Nullability.nullable);
  ...
  @override
  int get hashCode => _combineHashes(2059, nullability.index);

  @override
  bool operator ==(other) =>
      other is _NullTypeHandle;

  @override
  String toString() => 'Null';
}

class _SimpleTypeHandle extends TypeHandle {
  _ClassHandle? class_;
  ...
  @override
  void readContents(BufferedReader reader) {
    class_ = reader.readPackedObject() as _ClassHandle;
  }
...
class _TypeParameterHandle extends TypeHandle {
  ObjectHandle? parent;
  ...
  @override
  void readContents(BufferedReader reader) {
    parent = reader.readPackedObject() as ObjectHandle?;
...
class _GenericTypeHandle extends TypeHandle {
  ...
  void readContents(BufferedReader reader) {
    class_ = reader.readPackedObject() as _ClassHandle;
    typeArgs = reader.readPackedObject() as _TypeArgumentsHandle;
  }
...
class _RecursiveGenericTypeHandle extends TypeHandle {
  int? id;
  late _ClassHandle class_;
  late _TypeArgumentsHandle? typeArgs;
...
class _RecursiveTypeRefHandle extends TypeHandle {
  late int id;
...
class _FunctionTypeHandle extends TypeHandle {
  ...
  late List<NameAndType> typeParams;
  late int numRequiredParams;
  late List<TypeHandle> positionalParams;
  late List<NameAndType> namedParams;
  late TypeHandle returnType;
  ...
  void readContents(BufferedReader reader) {
    ...
    positionalParams = new List<TypeHandle>.generate(
        hasNamedParams ? numRequiredParams : numParams,
        (_) => reader.readPackedObject() as TypeHandle);
    if (hasNamedParams) {
      namedParams = new List<NameAndType>.generate(
          numParams - numRequiredParams,
          (_) => new NameAndType(
              reader.readPackedObject() as NameHandle, reader.readPackedObject() as TypeHandle));
    } else {
      namedParams = const <NameAndType>[];
    }
    returnType = reader.readPackedObject() as TypeHandle;
  }
...
abstract class NameHandle extends ObjectHandle {
...
class _PublicNameHandle extends NameHandle {
  late String name;
  ...
  @override
  int get flags => NameHandle.flagIsPublic;
...
class _PrivateNameHandle extends NameHandle {
  late _LibraryHandle library;
  late String name;
...
class _TypeArgumentsHandle extends ObjectHandle {
  ...
  @override
  void readContents(BufferedReader reader) {
    args = reader.readPackedList<TypeHandle>();
  }
...
class _ConstObjectHandle extends ObjectHandle {
  late ConstTag tag;
  dynamic value;
  ObjectHandle? type; // Value
  ObjectHandle? typeKey;
  int _hashCode = 0;

  _ConstObjectHandle._empty();

  _ConstObjectHandle(this.tag, this.value, [this.type, this.typeKey]);
  ...
  void writeContents(BufferedWriter writer) {
    switch (tag) {
      ...
      case ConstTag.kMap:
        {
          final items = value as Map<ObjectHandle, ObjectHandle>;
          writer.writePackedObject(type!);
          writer.writePackedObject(typeKey!);
          writer.writePackedUInt30(items.length);
          items.forEach((ObjectHandle key, ObjectHandle value) {
            writer.writePackedObject(key);
            writer.writePackedObject(value);
          });
        }
        break;
  ...
  void readContents(BufferedReader reader) {
    switch (tag) {
      ...
      case ConstTag.kBool:
        ...
        break;
      case ConstTag.kInstance:
        type = reader.readPackedObject() as ObjectHandle?;
        value = Map<ObjectHandle, ObjectHandle>.fromEntries(
            new List<MapEntry<ObjectHandle, ObjectHandle>>.generate(
                ...
                (_) => new MapEntry<ObjectHandle, ObjectHandle>(
                    reader.readPackedObject() as ObjectHandle, reader.readPackedObject() as ObjectHandle)));
        break;
      case ConstTag.kList:
      case ConstTag.kSet:
        type = reader.readPackedObject() as ObjectHandle?;
      ...
      case ConstTag.kMap:
        type = reader.readPackedObject() as ObjectHandle?;
        typeKey = reader.readPackedObject() as ObjectHandle?;
        value = Map<ObjectHandle, ObjectHandle>.fromEntries(
            new List<MapEntry<ObjectHandle, ObjectHandle>>.generate(
                reader.readPackedUInt30(),
                (_) => new MapEntry<ObjectHandle, ObjectHandle>(
  ...
  void accountUsesForObjectCopies(int numCopies) {
    switch (tag) {
      ...
      case ConstTag.kMap:
        {
          type!._useCount += numCopies;
          typeKey!._useCount += numCopies;
          final items = value as Map<ObjectHandle, ObjectHandle>;
          items.forEach((ObjectHandle key, ObjectHandle value) {
            key._useCount += numCopies;
            value._useCount += numCopies;
          });
        }
        break;
      ...
      case ConstTag.kMap:
        {
          final items = value as Map<ObjectHandle, ObjectHandle>;
          return _hashCode =
              _combineHashes(typeKey.hashCode, _combineHashes(type.hashCode, mapHashCode(items)));
        }
        break;
      case ConstTag.kTearOffInstantiation:
  ...
  bool operator ==(other) {
    ...
    if (other is _ConstObjectHandle && this.tag == other.tag) {
      switch (tag) {
        ...
        case ConstTag.kMap:
          return this.type == other.type && this.typeKey == other.typeKey && mapEquals(this.value, other.value);
        case ConstTag.kTearOffInstantiation:
  ...
  String toString() {
    switch (tag) {
      ...
      case ConstTag.kSet:
        return 'const <$type> $value';
      case ConstTag.kMap:
        return 'const <$typeKey, $type> $value';
      case ConstTag.kTearOff:
...
class _ScriptHandle extends ObjectHandle {
  ...
  @override
  void readContents(BufferedReader reader) {
    uri = reader.readPackedObject() as ObjectHandle;
    if ((_flags & flagHasSourceFile) != 0) {
...
class ObjectTable implements ObjectWriter, ObjectReader {
  ...
  final List<ObjectHandle> _objects = [];
  ...
  late TypeHandle _dynamicType;
  late TypeHandle _voidType;
  late TypeHandle _nullType;
  late _NodeVisitor _nodeVisitor;

  ObjectTable(CoreTypes coreTypes) {
    _dynamicType = getOrAddObject(new _DynamicTypeHandle()) as TypeHandle;
    _voidType = getOrAddObject(new _VoidTypeHandle()) as TypeHandle;
    _nullType = getOrAddObject(new _NullTypeHandle()) as TypeHandle;
  ...
  List<ObjectHandle> getHandles(List<Node> nodes) {
    final handles = List.generate(nodes.length, (i) => getHandle(nodes[i])!);
    return handles;
  }
  ...
  ObjectHandle getPublicNameHandle(String? name) {
    ...
    if (handle == null) {
      handle = getOrAddObject(new _PublicNameHandle(name)) as _PublicNameHandle;
  ...
  ObjectHandle getNameHandle(Library? library, String name) {
    ...
    return getOrAddObject(new _PrivateNameHandle(libraryHandle as _LibraryHandle, name));
  }
  ...
  List<_PublicNameHandle> getPublicNameHandles(List<String> names) {
    ...
    final handles = new List<_PublicNameHandle>.filled(names.length, 0 as _PublicNameHandle);
    for (int i = 0; i < names.length; ++i) {
      handles[i] = getPublicNameHandle(names[i]) as _PublicNameHandle;
    }
    return handles;
  }
  ...
  List<ObjectHandle> getConstStringHandles(List<String> values) {
    ...
    final handles = new List<ObjectHandle>.filled(values.length, 0 as ObjectHandle);
  ...
  ObjectHandle getTopLevelClassHandle(Library library) {
    ...
    return getOrAddObject(new _ClassHandle(libraryHandle as _LibraryHandle, name as NameHandle));
  }
  ...
  ObjectHandle getMemberHandle(Member member,
    ...
    return getOrAddObject(
        new _MemberHandle(classHandle as _ClassHandle, nameHandle as NameHandle, isField, isConstructor));
  }
  ...
  ObjectHandle? getTypeArgumentsHandle(List<DartType>? typeArgs) {
    ...
    List<TypeHandle> handles = List.generate(typeArgs.length, (i) => getHandle(typeArgs[i]) as TypeHandle);
  ...
  ObjectHandle getArgDescHandleByArguments(Arguments args,
    ...
    if (namedArguments.isNotEmpty) {
      argNames = List.generate(namedArguments.length, (i) => getPublicNameHandle(namedArguments[i].name) as _PublicNameHandle);
    }
  ...
  ObjectHandle getScriptHandle(Uri uri, SourceFile? source) {
    ...
    _ScriptHandle handle = getOrAddObject(new _ScriptHandle(uriHandle, source)) as _ScriptHandle;
  ...
  List<NameAndType> getTypeParameterHandles(List<TypeParameter> typeParams) {
    ...
    List<NameAndType> namesAndBounds = [];
    for (TypeParameter tp in typeParams) {
      namesAndBounds.add(
          new NameAndType(getPublicNameHandle(tp.name) as NameHandle, getHandle(tp.bound) as TypeHandle));
    }
  ...
  void declareClosure(
    ...
    final handle = getOrAddObject(
        new _ClosureHandle(getHandle(enclosingMember) as _MemberHandle, closureIndex));
  ...
  void allocateIndexTable() {
    ...
    _indexTable = [];
    int count = 1;
    _indexTable!.add(new _InvalidHandle()
  ...
  BytecodeObject? readObject(BufferedReader reader) {
    ...
    if ((header & ObjectHandle.referenceBit) == 0) {
      ...
      return ((index == 0) ? null : _indexTable![index]) as BytecodeObject;
    }
  }
  ...
  void write(BufferedWriter writer) {
    ...
    List<int> offsets = new List<int>.filled(_indexTable!.length, 0);
  ...
  ObjectTable.read(BufferedReader reader) {
    ...
    for (int i = 0; i < numEntries; ++i) {
      final int header = reader.readPackedUInt30();
      _indexTable!.add(new ObjectHandle._read(reader, header)
...
class _NodeVisitor extends Visitor<ObjectHandle?> {
  ...
  ObjectHandle visitLibrary(Library node) {
    ...
    return objectTable.getOrAddObject(new _LibraryHandle(uri as _ConstObjectHandle));
  }
  ...
  ObjectHandle visitClass(Class node) {
    ...
    final name = node.name.startsWith('_')
        ? objectTable.getOrAddObject(new _PrivateNameHandle(library as _LibraryHandle, node.name))
        : objectTable.getPublicNameHandle(node.name);
    return objectTable.getOrAddObject(new _ClassHandle(library as _LibraryHandle, name as NameHandle));
  }
  ...
  ObjectHandle visitInterfaceType(InterfaceType node) {
    ...
    if (!hasInstantiatorTypeArguments(node.classNode)) {
      return objectTable
          .getOrAddObject(new _SimpleTypeHandle(classHandle as _ClassHandle?, node.nullability));
    }
    ...
    final result = objectTable.getOrAddObject(isRecursive
        ? new _RecursiveGenericTypeHandle(
            recursiveId, classHandle as _ClassHandle, typeArgsHandle as _TypeArgumentsHandle?, node.nullability)
        : new _GenericTypeHandle(
            classHandle as _ClassHandle, typeArgsHandle as _TypeArgumentsHandle?, node.nullability));
  ...
  ObjectHandle visitFutureOrType(FutureOrType node) {
    ...
    final result = objectTable.getOrAddObject(
        new _GenericTypeHandle(classHandle as _ClassHandle, typeArgsHandle as _TypeArgumentsHandle, node.nullability));
    return result;
  }
  ...
  ObjectHandle visitFunctionType(FunctionType node) {
    ...
    final List<TypeHandle> positionalParams = [];
    for (var param in node.positionalParameters) {
      positionalParams.add(objectTable.getHandle(param) as TypeHandle);
    }
    final List<NameAndType> namedParams = [];
    for (var param in node.namedParameters) {
      namedParams.add(new NameAndType(
          objectTable.getPublicNameHandle(param.name) as NameHandle,
          objectTable.getHandle(param.type) as TypeHandle));
    }
  ...
  @override
  ObjectHandle visitNullConstant(NullConstant node) => objectTable._nullType;
  ...
  ObjectHandle visitMapConstant(MapConstant node) {
    return objectTable.getOrAddObject(new _ConstObjectHandle(
          ConstTag.kMap,
          Map.fromEntries(node.entries.map((ConstantMapEntry item) => new MapEntry(
              objectTable.getHandle(item.key)!,
              objectTable.getHandle(item.value)!))),
          objectTable.getHandle(node.valueType),
          objectTable.getHandle(node.keyType)));
  }
  ...
  @override
  ObjectHandle defaultConstantReference(Constant node) {
    // TODO: implement defaultConstantReference
    throw UnimplementedError();
  }

  @override
  ObjectHandle defaultMemberReference(Member node) {
    // TODO: implement defaultMemberReference
    throw UnimplementedError();
  }

  @override
  ObjectHandle visitClassReference(Class node) {
    // TODO: implement visitClassReference
    throw UnimplementedError();
  }

  @override
  ObjectHandle visitExtensionReference(Extension node) {
    // TODO: implement visitExtensionReference
    throw UnimplementedError();
  }

  @override
  ObjectHandle visitTypedefReference(Typedef node) {
    // TODO: implement visitTypedefReference
    throw UnimplementedError();
  }
}

...
#define GET_FIELD(instance, offset)                                            \
  reinterpret_cast<CompressedTypeArgumentsPtr*>(                               \
      reinterpret_cast<uword>(instance->untag()) + (offset))                   \
      ->Decompress(instance->untag()->heap_base())

#define GET_FIELD_BY_INDEX(instance, idx)                                      \
  GET_FIELD(instance, kCompressedWordSize*(idx))

#define SET_FIELD(instance, offset, value)                                     \
  *reinterpret_cast<CompressedObjectPtr*>(                                     \
      reinterpret_cast<uword>(instance->untag()) + (offset)) = value

#define SET_FIELD_BY_INDEX(instance, idx, value)                               \
  SET_FIELD(instance, kCompressedWordSize*(idx), value)
  ...
  DART_FORCE_INLINE static intptr_t ArgDescTypeArgsLen(ArrayPtr argdesc) {
    return Smi::Value(*reinterpret_cast<CompressedSmiPtr*>(
...
DART_NOINLINE bool Interpreter::InvokeCompiled(Thread* thread,
  ...
  CodePtr code = function->untag()->code();
...
DART_FORCE_INLINE bool Interpreter::InvokeBytecode(Thread* thread,
  ...
  BytecodePtr bytecode = function->untag()->bytecode();
...
#define BUMP_USAGE_COUNTER_ON_ENTRY(function)                                  \
  {                                                                            \
    int32_t counter = ++(function->untag()->usage_counter_);                   \
...
bool Interpreter::AssertAssignable(Thread* thread,
  ...
  if (cache != null_value) {
    ...
    if (cid == kClosureCid) {
      ...
      instance_cid_or_function = closure->untag()->function();
    } else {
      ...
      if (instance_class->untag()->num_type_arguments_ < 0) {
        ...
        instance_type_arguments = GET_FIELD_BY_INDEX(
            instance, instance_class->untag()
                          ->host_type_arguments_field_offset_in_words_);
      }
      parent_function_type_arguments =
    ...
    int idx = 0;
    for (auto entries = cache->untag()->cache()->untag();
         entries->element(idx) != null_value;
         idx += SubtypeTestCache::kTestEntryLength) {
      if ((entries->element(idx + SubtypeTestCache::kInstanceCidOrSignature) ==
           instance_cid_or_function) &&
          (entries->element(idx + SubtypeTestCache::kInstanceTypeArguments) ==
           instance_type_arguments) &&
          (entries->element(idx +
                            SubtypeTestCache::kInstantiatorTypeArguments) ==
           instantiator_type_arguments) &&
          (entries->element(idx + SubtypeTestCache::kFunctionTypeArguments) ==
           function_type_arguments) &&
          (entries->element(
               idx + SubtypeTestCache::kInstanceParentFunctionTypeArguments) ==
           parent_function_type_arguments) &&
          (entries->element(
               idx + SubtypeTestCache::kInstanceDelayedFunctionTypeArguments) ==
           delayed_function_type_arguments)) {
        if (Bool::True().ptr() ==
            entries->element(idx + SubtypeTestCache::kTestResult)) {
          return true;
        } else {
          break;
        }
...
bool Interpreter::AssertAssignableField(Thread* thread,
  ...
  SubtypeTestCachePtr cache = field->untag()->type_test_cache();
...
ObjectPtr Interpreter::Call(const Function& function,
  ...
  return Call(function.ptr(), arguments_descriptor.ptr(), arguments.Length(),
              reinterpret_cast<ObjectPtr*>(arguments.untag()->data()), thread);
}
    ...
    if (LIKELY(Array::IsValidLength(length))) {
      ...
      if (TryAllocate(thread, kArrayCid, Array::InstanceSize(length),
        ...
        for (intptr_t i = 0; i < length; i++) {
          result->untag()->set_element(i, Object::null());
        }
        SP[0] = result;
        return true;
      }
...
bool Interpreter::AllocateContext(Thread* thread,
  ...
  if (TryAllocate(thread, kContextCid,
    ...
    for (intptr_t i = 0; i < num_context_variables; i++) {
      result->untag()->set_element(i, null_value);
    }
    SP[0] = result;
    return true;
  } else {
...
bool Interpreter::AllocateClosure(Thread* thread,
  ...
  if (TryAllocate(thread, kClosureCid, instance_size,
    ...
    for (intptr_t offset = sizeof(UntaggedInstance); offset < instance_size;
      ...
      *reinterpret_cast<CompressedObjectPtr*>(start + offset) = null_value;
    }
    SP[0] = result;
    return true;
  } else {
...
#undef TARGET
...

  {
    ...
    SP = FP + num_locals - 1;
    TRACE_OPT("EntryFixed %ld locals - %s\n", num_locals,
              Function::Handle(function).ToFullyQualifiedCString());
...

  {
    ...
    TRACE_OPT("EntryOptional A: %u, B: %u, C: %u - %s\n", rA, rB, rC,
              Function::Handle(function).ToFullyQualifiedCString());
...

  {
    ...
    // Invoke target function.
    {
      ...
      TRACE_OPT("DirectCall argc: %u, kidx: %u - %s\n", argc, kidx,
                Function::Handle(static_cast<FunctionPtr>(LOAD_CONSTANT(kidx)))
...

  {
    ...
    // Invoke target function.
    {
      const uint32_t argc = rF;
      const uint32_t kidx = rD;
      TRACE_OPT("UncheckedDirectCall argc: %u, kidx: %u\n", argc, kidx);
...

  {
    ...
    DEBUG_CHECK;
    {
      ...
      TRACE_OPT("InterfaceCall argc: %u, kidx: %u - %s\n", argc, kidx,
                Object::Handle(static_cast<ObjectPtr>(LOAD_CONSTANT(kidx)))
                    .ToCString());
  ...
  }
  {
    ...
    DEBUG_CHECK;
    {
      ...
      ObjectPtr* call_base = SP - argc + 1;
      ObjectPtr* call_top = SP + 1;
      TRACE_OPT("InstantiatedInterfaceCall argc: %u, kidx: %u\n", argc, kidx);
...

  {
    ...
    DEBUG_CHECK;
    {
      ...
      ObjectPtr* call_base = SP - argc + 1;
      ObjectPtr* call_top = SP + 1;
      TRACE_OPT("UncheckedClosureCall argc: %u, kidx: %u\n", argc, kidx);
...

  {
    ...
    DEBUG_CHECK;
    {
      ...
      ObjectPtr* call_base = SP - argc + 1;
      ObjectPtr* call_top = SP + 1;
      TRACE_OPT("UncheckedInterfaceCall argc: %u, kidx: %u\n", argc, kidx);
...

  {
    ...
    DEBUG_CHECK;
    {
      ...
      ObjectPtr* call_base = SP - argc + 1;
      ObjectPtr* call_top = SP + 1;
      TRACE_OPT("DynamicCall argc: %u, kidx: %u\n", argc, kidx);
...

  {
    ...
    switch (kind) {
      ...
      case MethodRecognizer::kGrowableArrayCapacity: {
        GrowableObjectArrayPtr instance =
            static_cast<GrowableObjectArrayPtr>(SP[0]);
        SP[0] = instance->untag()->data()->untag()->length();
      } break;
      ...
      case MethodRecognizer::kLinkedHashBase_getIndex: {
        InstancePtr instance = static_cast<InstancePtr>(SP[0]);
        SP[0] = reinterpret_cast<ObjectPtr*>(
            instance->untag())[LinkedHashBase::index_offset() / kWordSize];
      } break;
      case MethodRecognizer::kLinkedHashBase_setIndex: {
        ...
        instance->untag()->StorePointer(
            reinterpret_cast<ObjectPtr*>(instance->untag()) +
                LinkedHashBase::index_offset() / kWordSize,
            SP[0]);
        *--SP = null_value;
      } break;
      case MethodRecognizer::kLinkedHashBase_getData: {
        InstancePtr instance = static_cast<InstancePtr>(SP[0]);
        SP[0] = reinterpret_cast<ObjectPtr*>(
            instance->untag())[LinkedHashBase::data_offset() / kWordSize];
      } break;
      case MethodRecognizer::kLinkedHashBase_setData: {
        InstancePtr instance = static_cast<InstancePtr>(SP[-1]);
        instance->untag()->StorePointer(
            reinterpret_cast<ObjectPtr*>(instance->untag()) +
                LinkedHashBase::data_offset() / kWordSize,
            SP[0]);
        *--SP = null_value;
      } break;
      case MethodRecognizer::kLinkedHashBase_getHashMask: {
        InstancePtr instance = static_cast<InstancePtr>(SP[0]);
        SP[0] = reinterpret_cast<ObjectPtr*>(
            instance->untag())[LinkedHashBase::hash_mask_offset() / kWordSize];
      } break;
      ...
      case MethodRecognizer::kLinkedHashBase_getUsedData: {
        InstancePtr instance = static_cast<InstancePtr>(SP[0]);
        SP[0] = reinterpret_cast<ObjectPtr*>(
            instance->untag())[LinkedHashBase::used_data_offset() / kWordSize];
      } break;
      ...
      case MethodRecognizer::kLinkedHashBase_getDeletedKeys: {
        ...
        SP[0] = reinterpret_cast<ObjectPtr*>(
            instance
                ->untag())[LinkedHashBase::deleted_keys_offset() / kWordSize];
      } break;
...
#undef TYPED_DATA_FACTORY
...

  {
    ...
    SET_FIELD_BY_INDEX(instance, offset_in_words, Object::sentinel().ptr());
...

  {
    ...
    if (unboxing && (guarded_cid == kDoubleCid) && supports_unboxed_doubles_) {
      ...
      SET_FIELD_BY_INDEX(instance, offset_in_words, value);
      TRACE_OPT("StoreFieldTOS: %p[%ld] <= %lx\n", instance->untag(),
                offset_in_words, (intptr_t)value);
    }
...

  {
    ...
    instance->untag()->set_element(variable, value);
    TRACE_OPT("StoreContextVar: %p[%d] <= %lx\n", instance->untag(), variable,
              (intptr_t)value);
...
#endif
    InstancePtr instance = static_cast<InstancePtr>(SP[0]);
    SP[0] = GET_FIELD_BY_INDEX(instance, offset_in_words);
    TRACE_OPT("LoadFieldTOS: %p[%ld] => %lx\n", instance->untag(),
              offset_in_words, (intptr_t)SP[0]);
...

  {
    ...
    int variable = rE;
    ASSERT(rE < static_cast<uint32_t>(instance->untag()->num_variables_));
    SP[0] = instance->untag()->element(variable);
    TRACE_OPT("LoadContextVar: %p[%d] => %lx\n", instance->untag(), variable,
              (intptr_t)SP[0]);
    DISPATCH();
  }

  {
    ...
    TRACE_OPT("AllocateContext: %u vars => %lx\n", num_context_variables,
              (intptr_t)SP[0]);
    DISPATCH();
  }
...

  {
    ...
    if (LIKELY(InterpreterHelpers::IsAllocateFinalized(cls))) {
      ...
      if (TryAllocate(thread, class_id, instance_size, &result)) {
        ...
        *++SP = result;
        TRACE_OPT("Allocate: %ld bytes => %lx\n", instance_size,
                  (intptr_t)result);
        DISPATCH();
      }
    }
    ...
    if (LIKELY(InterpreterHelpers::IsAllocateFinalized(cls))) {
      ...
      if (TryAllocate(thread, class_id, instance_size, &result)) {
        ...
        const intptr_t type_args_offset =
            ...
            kCompressedWordSize;
        *reinterpret_cast<CompressedObjectPtr*>(start + type_args_offset) =
            type_args;
        *--SP = result;
        DISPATCH();
      }
...

  {
    ...
    array->untag()->set_element(Smi::Value(index), value);
    TRACE_OPT("StoreIndexedTOS: %p[%ld] <= %lx\n", array->untag(),
              Smi::Value(index), (intptr_t)value);
    DISPATCH();
  }
...
#undef UNIMPLEMENTED_LABEL
...

  {
    ...
    FieldPtr field = static_cast<FieldPtr>(function->untag()->data());
    intptr_t offset_in_words =
        Smi::Value(field->untag()->host_offset_or_field_id_);
    ...
    ObjectPtr value = GET_FIELD_BY_INDEX(instance, offset_in_words);
    ...
    if (UNLIKELY(value == Object::sentinel().ptr())) {
      ...
      value = GET_FIELD_BY_INDEX(instance, offset_in_words);
    }
    //OS::PrintErr("ImplicitGetter: %p[%ld] => %lx\n", instance->untag(), offset_in_words, (intptr_t)value);
    ...
    *++SP = value;
    ...
    if (UNLIKELY(
      ...
      // Reload objects after the call which may trigger GC.
      field = static_cast<FieldPtr>(FrameFunction(FP)->untag()->data());
...

  {
    ...
    if (unboxing && (guarded_cid == kDoubleCid) && supports_unboxed_doubles_) {
      ...
      simd128_value_t raw_value;
      raw_value.readFrom(Float32x4::RawCast(value)->untag()->value_);
      Float32x4Ptr box = *(reinterpret_cast<Float32x4Ptr*>(instance->untag()) +
                           offset_in_words);
      ASSERT(box != null_value);  // Non-initializing store.
      ...
      simd128_value_t raw_value;
      raw_value.readFrom(Float64x2::RawCast(value)->untag()->value_);
      Float64x2Ptr box = *(reinterpret_cast<Float64x2Ptr*>(instance->untag()) +
                           offset_in_words);
      ASSERT(box != null_value);  // Non-initializing store.
      raw_value.writeTo(box->untag()->value_);
    } else {
      SET_FIELD_BY_INDEX(instance, offset_in_words, value);
      //OS::PrintErr("ImplicitSetter: %p[%ld] <= %lx\n", instance->untag(), offset_in_words, (intptr_t)value);
    }
...

  {
    ...
    if (value == Object::sentinel().ptr() ||
      ...
      // Reload objects after the call which may trigger GC.
      function = FrameFunction(FP);
      field = static_cast<FieldPtr>(function->untag()->data());
...

  {
    ...
    context->untag()->set_element(0, instance);
    ...
    closure->untag()->set_function(
        Function::RawCast(FrameFunction(FP)->untag()->data()));
...

  {
    ...
    for (intptr_t i = 0; i < argc; i++) {
      array->untag()->set_element(i, argv[i]);
    }
...

  {
    ...
    ASSERT(Function::Handle(target).kind() !=
           UntaggedFunction::kDynamicInvocationForwarder);
    BytecodePtr target_bytecode = target->untag()->bytecode();
    ...
    if (KernelBytecode::IsEntryOptionalOpcode(pc2)) {
      pp_ = target_bytecode->untag()->object_pool();
      uint32_t rA, rB, rC;
    ...
    intptr_t len = Smi::Value(checks->untag()->length_);
    SP[1] = checks;
    SP[2] = argdesc_;
    ...
    if (type_args_len > 0) {
      ...
      SP[6] = TypeArguments::RawCast(checks->untag()->element(1));
    ...
    for (intptr_t i = 2; i < len; i++) {
      ParameterTypeCheckPtr check =
          ParameterTypeCheck::RawCast(checks->untag()->element(i));

      if (LIKELY(check->untag()->index_ != 0)) {
        ASSERT(&FP[check->untag()->index_] <= SP);
        SP[3] = Instance::RawCast(FP[check->untag()->index_]);
        // TODO(regis): Revisit null handling once interpreter supports NNBD.
        if (SP[3] == null_value) {
          continue;  // Not handled by AssertAssignable for some reason...
        }
        SP[4] = check->untag()->type_or_bound_;
        // SP[5]: Instantiator type args.
        // SP[6]: Function type args.
        SP[7] = check->untag()->name_;
        ...
        // SP[5]: Instantiator type args.
        // SP[6]: Function type args.
        SP[7] = check->untag()->param_;
        SP[8] = check->untag()->type_or_bound_;
        SP[9] = check->untag()->name_;
        SP[10] = 0;
    ...
    target = Function::RawCast(checks->untag()->element(0));

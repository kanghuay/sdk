 ...
 private:
  ...
  class FunctionTypeScope : public ValueObject {
   ...
   private:
    ...
    const TypeParameters* const saved_type_parameters_;
  };
  ...
  FunctionTypePtr ReadFunctionSignature(const Function& func,
  ...
  bool IsExpressionEvaluationLibrary(const Library& library) const {
    return expression_evaluation_library_ != nullptr &&
           expression_evaluation_library_->ptr() == library.ptr();
  }
  ...
  const TypeParameters* function_type_type_parameters_ = nullptr;
...
class BytecodeComponentData : ValueObject {
 public:
  ...
  void Init(const Array& data) { data_ = data.ptr(); }
...
class BytecodeReader : public AllStatic {
 public:
  ...
  static ObjectPtr ReadAnnotation(const Library& library, intptr_t bytecode_offset);
  ...
  static ArrayPtr ReadExtendedAnnotations(const Library& library, intptr_t bytecode_offset,
                                          intptr_t count);

...
class Simulator {
 public:
  ...
  int64_t get_register(Register reg) const { return registers_[reg]; }
  int64_t get_sp() const { return registers_[SPREG]; }
  int64_t get_lr() const { return registers_[R30]; }

  uint64_t get_pc() const { return pc_; }
  uint64_t get_last_pc() const { return last_pc_; }
